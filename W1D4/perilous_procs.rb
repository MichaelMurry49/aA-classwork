...
def filter_out (arr, &proc)
    res = []
    arr.each {|ele| res << ele if !proc.call(ele)}
    res
end
...
def first_index(arr, &proc)
    arr.each_with_index { |el, idx| return idx if proc.call(el)}
    nil
end
...
def proctition (arr, &proc)
    arr.select{|ele| proc.call(ele)}.concat(arr.reject{|ele| proc.call(ele)})
end
...
def selected_map! (arr, proc1, proc2)
    arr.map { |ele| (proc1.call(ele)) ? proc2.call(ele) : ele}
...
def proc_suffix(sentence, hash)
    ...
    words.each_with_index do |word, idx|
        hash.each { |k,v| words[idx] += v if k.call(word)}
    end
    words.join(' ')
end
...
def proctition_platinum(array, *procs)
    hash = {}
    (1..procs.length).each do |key| 
        hash[key] = array.select { |ele| procs[key - 1].call(ele)}
        array = array.reject { |ele| procs[key - 1].call(ele)}
end
    hash
end
...
# puts "WHERE!" == "WHERE!".upcase

def procipher (sentence, hash)
    words = sentence.split(" ")
    new_words = [*words]
    hash.each do |k,v| 
        words.each_with_index {|word, i| new_words[i] = v.call(new_words[i]) if k.call(word)}
    end
    new_words.join(" ")
end
